...
export const Todo = () => {
  ...
  <div className="w-full px-2 sm:px-4">
    ...
    <div className="max-h-[70vh] h-full shadow-lg border-t-2 rounded border-indigo-900 bg-white overflow-hidden flex flex-col">
      ...
      <div className="mx-2 mt-2 overflow-x-auto max-h-[28.4rem]">
        <div className="min-w-[900px]">
          {/* Table Header */}
          <div
            className="grid grid-cols-[0.5fr_1fr_2fr_1fr_1fr_1fr_1fr] items-center
          ...
          {paginatedTodos.length === 0 ? (
            ...
            paginatedTodos.map((todo, index) => {
              ...
              return (
                <div
                  key={todo.id}
                  className="grid grid-cols-[0.5fr_1fr_2fr_1fr_1fr_1fr_1fr]
                  border border-gray-300 text-gray-800 text-sm p-2 items-center
                  hover:bg-gray-100 transition items-center"
                >
                  <span>{(pageNo - 1) * rowsPerPage + index + 1}</span>
                  {currentUser?.role === "admin" && (
                    <span className="truncate">{todo.employeeName ?? "-"}</span>
                  )}
                  ...
                  <span className="flex flex-nowrap items-center justify-center gap-1">
                    ...
                    <DeleteButton handleDelete={() => handleClickDeleteButton(todo.id)} />
                  </span>
                </div>
              );
            })
    ...
    {viewTodo && (
      <ViewTodo setIsOpenModal={() => setViewTodo(null)} viewTodo={viewTodo} />
    )}
