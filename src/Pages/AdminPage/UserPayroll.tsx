...
// Define Tab Types
type TabType = "LOAN_MANAGEMENT";
const entriesOptions = [5, 10, 15, 20, 30];

export const UserPayroll = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedValue, setSelectedValue] = useState(10);
  const [activeTab] = useState<TabType>("LOAN_MANAGEMENT");

  const [triggerModal, setTriggerModal] = useState<{
    tab: TabType;
    count: number;
  }>({ tab: "LOAN_MANAGEMENT", count: 0 });
      ...
      <div className="min-h-screen w-full flex flex-col shadow-lg bg-white rounded-md">
        ...
        <div className="px-4 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
          <div className="flex w-full sm:w-auto p-1 bg-[#F1F5F9] rounded-xl border border-gray-200">
            {(["LOAN_MANAGEMENT"] as TabType[]).map((tab) => (
              <button
                key={tab}
                className="flex-1 sm:flex-none px-4 sm:px-6 py-2 text-sm font-bold transition-all duration-200 rounded-lg bg-white text-[#334155] shadow-sm"
              >
                Loan
              </button>
            ))}
